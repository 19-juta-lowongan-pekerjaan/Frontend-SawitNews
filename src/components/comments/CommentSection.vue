  ...
  <div class="comment-section space-y-6">
    ...
    <div v-if="!commentStore.loading" class="space-y-6">
      ...
      >
          ...
          <div class="flex items-center gap-2.5">
            ...
            <div class="flex flex-col leading-tight">
              <div class="flex items-center gap-1.5">
                ...
                <!-- Badge icon -->
                <span class="w-4 h-4 flex items-center justify-center" :title="translateRank(comment.author?.rank)" v-if="comment.author?.rank" v-html="getBadge(comment.author.rank).icon"></span>
              </div>
        ...
        <!-- Comment content -->
        <p class="text-sm text-slate-700 dark:text-slate-300 pl-1">
          {{ comment.content }}
        </p>

        <!-- Actions Footer -->
        <div class="flex items-center gap-4 text-xs font-semibold pl-1">
          <!-- Like Comment -->
          <button 
            @click="likeComment(comment.id)"
            :class="['flex items-center gap-1 transition-colors', comment.isLiked ? 'text-red-500' : 'text-slate-400 hover:text-red-500']"
          >
            <svg class="h-3.5 w-3.5 fill-current" viewBox="0 0 24 24">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg>
            <span :class="[comment.isLiked ? 'text-red-500' : 'dark:text-slate-400 text-slate-500']">{{ comment.likes }}</span>
          </button>
          ...
          <button 
            ...
            class="text-red-400 hover:text-red-600 transition-colors ml-auto flex items-center gap-1"
          >
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
            {{ uiStore.t('delete_btn') }}
          </button>
        </div>

        <!-- Nested Inline Reply Input -->
        <div v-if="activeReplyId === comment.id" class="pt-2 pl-4 border-l-2 border-primary/20 space-y-2">
          <input 
            type="text" 
            v-model="replyText" 
            :placeholder="uiStore.t('reply_placeholder')"
            class="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-primary dark:bg-slate-900 dark:border-slate-800 dark:text-white"
            @keyup.enter="handlePostReply(comment)"
          />
          <div class="flex justify-end gap-2">
            <button @click="activeReplyId = null" class="px-2.5 py-1 text-xxs font-bold text-gray-500 bg-gray-50 border rounded-lg hover:bg-gray-100 dark:bg-slate-900 dark:border-slate-800">
              {{ uiStore.t('cancel_btn') }}
            </button>
            <button @click="handlePostReply(comment)" :disabled="!replyText.trim()" class="px-2.5 py-1 text-xxs font-bold text-white bg-primary hover:bg-primary-hover rounded-lg shadow disabled:opacity-50">
              {{ uiStore.t('reply_btn') }}
            </button>
          </div>
        </div>
        ...
        <div v-if="comment.replies && comment.replies.length > 0" class="pl-6 pt-3 border-t border-gray-50 space-y-4 dark:border-slate-700/50">
          <div 
            ...
            :key="reply.id"
            class="bg-slate-50/50 rounded-2xl p-4 space-y-2 dark:bg-slate-900/30"
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <div class="flex flex-col leading-none">
                  <div class="flex items-center gap-1">
                    ...
                    <span class="w-3.5 h-3.5 flex items-center justify-center text-gray-500" :title="translateRank(reply.author?.rank)" v-html="getBadge(reply.author?.rank || 'Pemula').icon"></span>
                  </div>
                  <span class="text-xxs text-gray-400">@{{ reply.author?.username }}</span>
                </div>
              </div>
            ...
            <p class="text-xs text-slate-600 pl-1 dark:text-slate-300">
              {{ reply.content }}
            </p>

            <div class="flex items-center gap-3 text-xxs font-semibold pl-1">
              <button 
                @click="likeComment(reply.id)"
                :class="['flex items-center gap-1 transition-colors', reply.isLiked ? 'text-red-500' : 'text-slate-400 hover:text-red-500']"
              >
                <svg class="h-3 w-3 fill-current" viewBox="0 0 24 24">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                </svg>
                <span :class="[reply.isLiked ? 'text-red-500' : '']">{{ reply.likes }}</span>
              </button>
            </div>
          </div>
        </div>
